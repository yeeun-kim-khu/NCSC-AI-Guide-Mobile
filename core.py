...
def route_intent(text: str) -> str:
    ...
    - "basic": 정적 FAQ — 운영시간·입장료·주차·오시는길 4가지만
    - "llm_agent": 그 외 모두 (LangGraph ReAct + RAG)
    """
    ...
    # 특정 날짜 휴관 여부 → LLM 도구 (날짜 계산 필요)
    if re.search(r"(\d+월\s*\d+일|\d{4}-\d{2}-\d{2}|내일|모레|이번\s*주|다음\s*주)", text) and any(
        t in lowered for t in ["열", "휴관", "운영", "여나", "여는", "쉬"]
    ):
        return "llm_agent"

    # 오늘/이번 주 교육프로그램 → LLM 도구 (날짜 계산 필요)
    if re.search(r"(오늘|이번\s*주|이번주|\d+월\s*\d+일|\d+일)", text) and any(
        t in lowered for t in ["교육", "프로그램", "수업", "교실"]
    ):
        return "llm_agent"

    # ── 4개 핵심 정적 FAQ만 rule-based ───────────────────────────────────────

    # 1. 운영시간 / 휴관일
    if any(k in lowered for k in [
        "운영시간", "운영 시간", "휴관", "휴무", "개관", "폐관",
        "몇 시에 열", "몇시에 열", "몇 시에 닫", "몇시에 닫",
        "언제 열", "언제 닫", "여는 시간", "닫는 시간", "여나요", "여나",
    ]):
        return "basic"

    # 2. 입장료 / 관람료
    if any(k in lowered for k in ["입장료", "관람료", "티켓값", "입장 요금", "관람 요금"]):
        return "basic"

    # 3. 주차
    if any(k in lowered for k in ["주차", "주차장", "주차비", "주차료"]):
        return "basic"

    # 4. 오시는 길 (출발지 없는 정적 교통 안내)
    if any(k in lowered for k in ["오시는길", "오는길", "오시는 길", "교통편", "어떻게 가", "어떻게 오", "가는 방법"]):
        return "basic"
...
##### 💡 생각놀이터 (1층)
- **주제**: 뇌의 역할과 원리를 알아보고, 나의 뇌 능력을 테스트하며 사고력을 키우는 전시관
- **추천 연령**: 초등 저~고학년
- **특징**: 브레인 헬스 클럽(운동과 뇌), 뇌가 크면 똑똑할까, 뉴런 연결, 알쏭달쏭 뇌과학 등 다양한 체험
- **주요 체험**: 반응속도 테스트, 기억력 테스트, 데자뷰 효과, 뇌과학 OX퀴즈
- **추천 체류시간**: 30~45分钟
...
- **특징**: 씨앗이 자라 나무가 되고, 나무들이 숲을 만드는 과정과 생태계 상호작용의 과학적 원리를 미디어 인터랙션으로 체험
- **구성**: 미디어 영상관 / 인트로 / 아웃트로
- **주요 체험**: 검은 지구와 식물의 탄생, 식물 전략(바람/물/동물 매개 씨앗 산포), 씨앗 날리기와 밟기 놀이, 신비한 식물 속으로(광합성/피보나치/프랙탈), 숲속 동식물 도감 수집(Catch and Collect 앱), 반달가슴곰 키넥트 체험
- **운영**: 회차제로 입장 (현장 안내 표지 확인)
- **상영 시간**: 약 15분
- **수용 인원**: 회차당 20명
- **위치**: 2층 빛놀이터 (구 4D영상관)
- **주의사항**: 어두운 환경으로 유아는 보호자 동반 필수, 36개월 미만 무료(증빙 필요), 상설전시관 예약 필수
