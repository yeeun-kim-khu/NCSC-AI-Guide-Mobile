...
def route_intent(text: str) -> str:
    ...
    clear_faq_keywords = [
        ...
        "층별", "층 안내",
        # 전시관
        "전시관", "놀이터 안내", "ai놀이터", "행동놀이터", "관찰놀이터", "탐구놀이터", "생각놀이터", "빛놀이터",
        # 연령별 동선
        ...
        "k-사이언스", "k사이언스",
        # 천체투영관 프로그램
        "코코몽", "키츠", "바니", "다이노소어", "슈퍼문", "우주정거장", "길냥이",
        # 과학쇼/사이언스랩/로봇쇼 (과학극장 프로그램)
        "사이언스랩", "과학쇼", "로봇쇼", "과학극장", "비눗방울", "드라이아이스", "팡팡공기",
        # 시설 편의 (화장실·휴게실)
        "화장실", "휴게실",
        # 외국어 FAQ 버튼 4종 (층별·프로그램·동선·전시관)
        "floor guide", "today's programs", "recommended route by age", "exhibition guide",
        "フロア案内", "今日のプログラム", "年齢別おすすめルート", "展示館案内",
        "楼层导览", "今日节目", "按年龄推荐路线", "展馆导览",
    ]
...
def classify_basic_category(message: str) -> str:
    ...
    rules = [
        # 가장 구체적인 카테고리 먼저
        ("education_guide", ["교육", "교육과정", "과학교실", "수학교실", "sw공학교실", "ai공학교실", "유아특화교실", "방학과정", "나눔과정", "창경궁프로그램", "창경궁교육", "빛놀이터교육", "전시연계교육", "어린이맞춤과학"]),
        ...
        ("route_by_age",    ["동선", "연령별", "연령", "나이", "추천코스", "추천동선", "추천해", "4~7", "7세", "유아", "초등", "저학년", "고학년", "몇살", "초등학생", "유아코스", "어린이코스", "관람코스", "살은", "세는", "살짜리", "세짜리", "살 아이", "세 아이", "살아이", "세아이", "recommended route by age", "年齢別おすすめルート", "按年龄推荐路线"]),
...
def adjust_tone_for_mode(text: str, mode: str, language: str = "한국어") -> str:
    """모드별 말투 조정 함수"""
    if mode == "어린이":
        if language == "한국어":
            ...
            # English child mode: add emojis, use simpler language
            if not any(emoji in text for emoji in ["😊", "🎉", "✨", "🤖", "🔬", "📐", "💻", "🧸", "🏛️", "📚", "🌿", "🍎", "🌈", "🎯", "⏰", "📋", "⚠️", "📞", "🎫", "📍", "📅"]):
                text = text.replace("Here is the information", "Here is the information! 😊")
                text = text.replace("information:", "information! 😊")
        elif language == "日本語":
            # Japanese child mode: add emojis, use friendly tone
            if not any(emoji in text for emoji in ["😊", "🎉", "✨", "🤖", "🔬", "📐", "💻", "🧸", "🏛️", "📚", "🌿", "🍎", "🌈", "🎯", "⏰", "📋", "⚠️", "📞", "🎫", "📍", "📅"]):
                text = text.replace("案内です", "案内です! 😊")
                text = text.replace("案内:", "案内! 😊")
        elif language == "中文":
            # Chinese child mode: add emojis, use friendly tone
            if not any(emoji in text for emoji in ["😊", "🎉", "✨", "🤖", "🔬", "📐", "💻", "🧸", "🏛️", "📚", "🌿", "🍎", "🌈", "🎯", "⏰", "📋", "⚠️", "📞", "🎫", "📍", "📅"]):
                text = text.replace("介绍", "介绍! 😊")
                text = text.replace("信息:", "信息! 😊")
    return text
...
#### ⚠️ 천체투영관 연속 관람 주의
빛놀이터와 천체투영관은 같은 시간대 연속 관람 불가 (예: 10시 천투 후 10:30 빛놀이터 불가). 예매 전 확인 필요. 동시 예매로 관람 불가 시 1층 매표소에서 환불 도와드립니다."""
            else:
                return """✨ **빛놀이터** 체험 안내입니다!
...
빛놀이터와 천체투영관은 같은 시간대 연속 관람 불가 (예: 10시 천투 후 10:30 빛놀이터 불가). 예매 전 확인 필요. 동시 예매로 관람 불가 시 1층 매표소에서 환불 도와드립니다."""
        ...
        if category == "science_show":
            ...
            # 특정 프로그램만 짧게 안내
            if mentioned_show and mode == "어린이":
                return f"""**{mentioned_show}**은(는) **1층 과학극장**에서 해요! 🎉

📍 **위치**: 1층 과학극장
⏰ **시간**: 11:30, 13:30 (하루 2회)
⌛ **소요시간**: 약 15분
🎫 **비용**: 무료 (선착순)

#### 🚶‍♂️ 이동 안내
- 바닥에 있는 **연두색 선**을 따라가시면 과학극장으로 갈 수 있어요!

#### 🚪 입장 절차 (중요!)
- **25분부터 입장 가능**하며, **입장 마감은 29분 59초까지** 입니다.
- **30분에 프로그램이 시작**되며, 시작된 후에는 관람객 입장을 허용하지 않습니다.

다른 과학쇼 프로그램 정보도 알려드릴까요?"""
            elif mentioned_show and mode == "성인":
                return f"""**{mentioned_show}**은(는) **1층 과학극장**에서 진행됩니다! 🎉
...
#### 🚶‍♂️ 이동 안내
- 바닥에 있는 **연두색 선**을 따라가시면 과학극장으로 갈 수 있습니다.
...
[🔗 홈페이지]({CSC_URLS.get('과학쇼', 'https://www.sciencecenter.go.kr/csc/cultural-event/science-show')})"""
                ...
                if _month % 2 == 0:
                    _show_name = "로봇쇼 「로봇 프렌즈!」"
                    _show_tip = f"🎉 **{_month}월은 로봇쇼 운영 중입니다!** 축구로봇·댄스로봇·로봇개 등 다양한 로봇을 만날 수 있습니다. 보러 가시겠습니까? **1층 과학극장**으로 오십시오! (11:30 / 13:30)"
                else:
                    _show_name = "사이언스랩"
                    _show_tip = f"🎉 **{_month}월은 사이언스랩 운영 중입니다!** 신기한 과학 실험을 직접 볼 수 있습니다. 보러 가시겠습니까? **1층 과학극장**으로 오십시오! (11:30 / 13:30)"
                return f"""🔬 과학쇼(로봇쇼·사이언스랩) 안내입니다!
...
#### 📅 연간 운영 일정
- **로봇쇼**: 짝수월 (2·4·6·8·10·12월)
  - 축구로봇, 댄스로봇, 로봇개 등 다양한 로봇의 구조와 작동 원리를 보여줍니다
- **사이언스랩**: 홀수월 (1·3·5·7·9·11월)
  - 월별 과학 실험 시연 프로그램 (비눗방울, 공기, 힘 등)
...
**1층 주황색 선**을 따라오시면 **천체투영관**에 도착합니다! 🟠
...
- **초1~2**: **2층 창작교실1**로 오십시오
- **유아(6세)**: **2층 창작교실2**로 오십시오"""
            elif any(p in lowered_msg for p in ["씨앗의 모험"]):
                if mode == "어린이":
                    return """씨앗의 모험 참여하러 왔군요! 🌱 씨앗이 어떻게 여행하는지 직접 알아보는 신기한 프로그램이에요!

**2층 창작교실2**로 오세요! (유아 친구들 대상 😊)"""
                else:
...
**2층 창작교실2**로 오시면 됩니다. (유아 대상)"""
                ...
                if mode == "어린이":
                    return """열매와 씨앗 모양에 숨은 비밀 참여하러 왔군요! 🍎 씨앗 모양이 왜 다 다른지 함께 알아봐요!

**2층 창작교실1**로 오세요! (초1~2 대상 😊)"""
                else:
...
- **유아·초1**: **1층 연두색 선**을 따라오십시오! 🟢
- **초2~4**: **2층 창작교실4**로 오십시오"""
            elif any(p in lowered_msg for p in ["과학교실", "과학 교실", "지구과학", "지구와 태양계"]):
                if mode == "어린이":
                    return """과학교실 참여하러 왔군요! 🔬 태양계와 우주의 비밀을 배우는 신나는 수업이에요!

**1층 어린이교실**로 오세요! (초1~4 대상 😊)"""
                else:
...
예약 확인서에서 정확한 교실 번호를 확인해 주십시오."""
            elif any(p in lowered_msg for p in ["k-사이언스", "k사이언스", "케이사이언스", "세종과 장영실", "똑딱똑딱", "시간 여행"]):
                if mode == "어린이":
                    return """K-사이언스 프로그램 참여하러 왔군요! 🏛️ 세종대왕과 장영실의 신기한 발명품을 만나봐요!
...
- **유아 대상**: **2층 창작교실1·2**로 오십시오
- **초등 대상**: **1층 어린이교실** 또는 **2층 창작교실1~4**로 오십시오

예약 확인서에서 정확한 교실을 확인해 주십시오."""
...
운영 장소는 프로그램에 따라 다릅니다. 담당자에게 문의해 주십시오.
📞 **문의**: 02-3668-3313"""
...
⏰ **운영**: 14:30 (화~금) · 약 15분 · 무료 · 선착순"""
                ...
                if mode == "어린이":
                    ...
                    return "찾으시는 프로그램 장소를 안내해 드리겠습니다! 프로그램명을 말씀해 주시면 바닥 선 색깔로 안내해 드리겠습니다. 😊"
        if category == "k_science":
            if mode == "어린이":
                return """**K-사이언스** 프로그램 안내예요! 😊
...
#### 📋 신청 방법
- **예약**: 신청콕 예약 (선착순)
- **신청 기간**: 5월 22일(금) 오후 2시부터 교육일 하루 전까지
...
#### 🤖 로봇순회 (추가 정보)
로봇쇼 외에도 **로봇순회**가 매일(화-일) 14:30에 진행됩니다! 다양한 로봇이 과학관을 돌아다니며 어린이들과 소통하는 특별한 체험이에요.
- **장소**: 1층 꿈트리동산
- **운영**: 매일 화-일 14:30
- **참여**: 자유롭게 참여 가능
...
| 시간 | 화 | 수 | 목 | 금 | 토 | 일 |
|---|---|---|---|---|---|---|
| 10:30 | 단체해설(10:40) | 단체해설(10:40) | 단체해설(10:40) | 단체해설(10:40) | 스폿해설 | 스폿해설 |
| 11:30 | 사이언스랩 | 사이언스랩 | 사이언스랩 | 사이언스랩 | 사이언스랩 | 사이언스랩 |
| 13:30 | 사이언스랩 | 사이언스랩 | 사이언스랩 | 사이언스랩 | 사이언스랩 | 사이언스랩 |
...
| 시간 | 화 | 수 | 목 | 금 | 토 | 일 |
|---|---|---|---|---|---|---|
| 10:30 | 단체해설(10:40) | 단체해설(10:40) | 단체해설(10:40) | 단체해설(10:40) | 전시톡톡해설 | 전시톡톡해설 |
| 11:30 | 로봇쇼 | 로봇쇼 | 로봇쇼 | 로봇쇼 | 로봇쇼 | 로봇쇼 |
| 13:30 | 로봇쇼 | 로봇쇼 | 로봇쇼 | 로봇쇼 | 로봇쇼 | 로봇쇼 |
...
#### 📋 신청 방법
- **예약**: 홈페이지 예약 → 신청콕 접속 교육 → 연속과정 → 교육일 선택 → 결제
- **문의**: 02-3668-3318
...
#### 📋 신청 방법
- **예약**: 신청콕 예약 (선착순)
- **신청 기간**: 해당월 22일(금) 11:00부터 교육일 하루 전까지
- **문의**: 02-3668-3350
...
#### 📋 신청 방법
- **예약**: 신청콕 예약 (선착순)
- **신청 기간**: 해당월 17일(금) 11:00부터 교육일 하루 전까지
- **문의**: 02-3668-3350
...
#### 📋 신청 방법
- **예약**: 홈페이지 예약 → 신청콕 접속 교육 → 연속과정 → 교육일 선택 → 결제
- **신청 기간**: 해당월 20일(수) 11시부터 해당월 5일(금) 23시 59분까지
- **문의**: 02-3668-3318
...
#### 🎯 프로그램 소개
방학과정은 여름방학(7-8월)·겨울방학(1-2월)에 운영되는 특별 교육 프로그램입니다. 방학 기간을 활용하여 과학을 더 깊이 있게 탐험할 수 있습니다.
...
아쉽게도 과학관 안에는 **공용 충전기가 없어요.**

#### 💡 어떻게 하면 될까요?
- 혜화역 근처 **편의점이나 카페**에서 충전할 수 있어요 😊
- 다음에 올 때 **보조배터리를 미리 챙겨오면** 더 편해요!

📞 문의: **02-3668-3350**"""
            else:
                return """🔋 휴대폰 충전 안내

국립어린이과학관에는 **별도의 공용 휴대폰 충전기가 없습니다.**

#### 💡 대안
- 근처 **편의점·카페**(혜화역 인근)에서 충전 가능
- 보조배터리를 챙겨오시면 더 편리합니다
...
아쉽지만 과학관에서는 **우산을 빌려드리지 않아요.**

#### 💡 비가 왔을 때는!
- **1층 안내데스크**에 먼저 여쭤보세요
- 혜화역 근처 **편의점**에서 우산을 살 수 있어요
- 입구 근처 **지붕 있는 곳**에서 잠깐 비를 피할 수 있어요

☔ 다음에 올 때는 우산을 미리 챙겨오세요! 😊
📞 문의: **02-3668-3350**"""
            else:
                return """☂️ 우산 대여 안내

현재 국립어린이과학관에서는 **우산 대여 서비스를 운영하지 않습니다.**

#### 💡 대안
- **1층 안내데스크**에서 긴급 상황 문의 가능
- 근처 **편의점**(혜화역 인근)에서 우산 구매 가능
- 입구 근처의 **지붕 있는 공간**에서 비를 잠시 피할 수 있습니다

☔ 비 오는 날 방문 시 우산을 챙겨오시면 편리합니다.
📞 문의: **02-3668-3350**"""

        if category == "late_arrival":
            if mode == "어린이":
                return """⏰ 프로그램에 늦었을 때 안내예요!

프로그램마다 들어갈 수 있는 시간이 달라요!

#### 🎭 과학쇼(사이언스랩·로봇쇼)·전시해설
- ✅ 회차 시작 **25분 전부터** 들어갈 수 있어요
- 🚫 회차 시작 후 **29분 59초가 지나면** 입장 안 돼요
- 🚫 공연 중에는 들어가거나 나올 수 없어요
- 다음 회차(11:30 또는 13:30)를 이용해 주세요 😊

#### 🌙 천체투영관
- ✅ **정각부터** 들어갈 수 있어요
- 🚫 **정각 + 9분이 지나면** 안전상 들어갈 수 없어요 (불이 꺼지고 시작해서!)
- 🚫 늦어서 못 보면 환불이 안 돼요
- 다음 회차는 매 정시에 출발해요 (09:30~16:30)

#### 💡 빛놀이터 (2층)
- 1시간마다 새 회차가 시작돼요 → 다음 회차 이용 가능
- 선착순이라 자리가 없을 수도 있어요

#### 📚 교육 프로그램(과학교실·수학교실 등)
- 예약 프로그램이라 늦으면 들어가기 어려워요
- 어쩔 수 없는 경우엔 1층 안내데스크에 먼저 여쭤보세요

📞 문의: **02-3668-3350**"""
            else:
                return """⏰ 프로그램 지각 시 입장 안내

프로그램마다 입장 마감 규정이 다릅니다.

#### 🎭 과학쇼 (사이언스랩·로봇쇼) · 전시해설
- ✅ **회차 시작 25분 전부터** 입장 가능
- 🚫 **회차 시작 후 29분 59초에 입장 마감** — 이후에는 입장 불가
- 🚫 프로그램 진행 중에는 입·퇴장 모두 불가
- 다음 회차(11:30 또는 13:30)를 이용해 주십시오

#### 🌙 천체투영관
- ✅ **회차 정각부터 입장 가능**
- 🚫 **정각 + 9분(09분)에 입장 마감** — 소등 후 안전상 입장 불가
- 🚫 늦어 관람 못한 경우 **환불 불가**
- 다음 회차는 매 정시에 출발합니다 (09:30~16:30)

#### 💡 빛놀이터 (2층)
- 1시간 간격으로 회차 운영 → 다음 회차 이용 가능
- 선착순이므로 자리가 없을 수도 있습니다

#### 📚 교육 프로그램 (과학교실·수학교실 등)
- 사전 예약 프로그램으로 **지각 입장 불가** 원칙
- 부득이한 경우 1층 안내데스크에 먼저 문의해 주십시오
        ...
        if category == "education_guide":
            ...
            if weekday == 5:  # 토요일
                if mode == "어린이":
                    ...
                    day_notice = f"오늘은 **{today_str}(토요일)**입니다! 🎉 교육 프로그램이 운영되는 날입니다.\n\n"
            else:
                ...
                if mode == "어린이":
                    ...
                    day_notice = (
                        f"오늘은 **{today_str}({weekday_kr}요일)**이라 정규 교육 프로그램 운영일이 아닙니다. 😊\n"
                        f"교육 프로그램(과학교실·수학교실 등)은 **매주 토요일**에 운영됩니다.\n"
                        f"다음 토요일은 **{next_sat_str}**입니다!\n\n"
                    )
            if mode == "어린이":
...
**상시 주말교육 (토요일)**
- 과학교실, SW공학교실(AI), 수학교실 (초등 1-4학년)

**특별교육**
- 방학과정 (여름/겨울방학)
- 나눔과정 (지역아동 및 장애아동)

> 💡 **어떤 프로그램이 더 궁금하신가요?** "과학교실 자세히", "천체투영관 시간표"처럼 물어보면 자세히 안내해드릴게요! 😊"""
            else:
                base = day_notice + """국립어린이과학관에서는 다양한 교육 프로그램을 운영하고 있습니다! 😊
...
> 💡 **어떤 프로그램이 더 궁금하신가요?** "과학교실 자세히", "천체투영관 시간표"처럼 물어보면 자세히 안내해드리겠습니다! 😊"""
...
> ⚡ **교육 신청 꿀팁**: 교육 신청 시작과 동시에 **수분 내 마감**되는 경우가 많아요! 신청 시작 시각에 바로 접속하시길 강력 추천드려요. 이 안내를 받으셨다면 **이미 예약이 마감됐을 수도 있으니**, 신청콕에서 잔여석을 꼭 확인해 보세요.
...
> ⚡ **교육 신청 꿀팁**: 교육 신청 시작과 동시에 **수분 내 마감**되는 경우가 많습니다. 신청 시작 시각에 바로 접속하시길 강력 추천드립니다. 이 안내를 받으셨다면 **이미 예약이 마감됐을 수도 있으니**, 신청콕에서 잔여석을 꼭 확인해 보세요.
...
1. 🦖 **다이노터널(전시물)** — 2층 관찰놀이터에 있는 초대형 공룡 터널이에요!
2. 🌌 **다이노소어(프로그램)** — 천체투영관에서 공룡이 살던 시대로 여행하는 영상이에요!

"전시물" 또는 "프로그램"이라고 말해주면 바로 안내해줄게요! 😊"""
            else:
                return """**다이노**에 대해 물어보셨군요! 정확한 안내를 위해 한 가지 확인이 필요합니다. 😊
...
"전시물" 또는 "프로그램"이라고 말씀해 주시면 상세히 안내드리겠습니다! 🦕"""
        ...
        if category == "floor_guide":
            ...
            if "1층" in lowered or "일층" in lowered:
                mentioned_floor = "1층"
            elif "2층" in lowered or "이층" in lowered:
                mentioned_floor = "2층"
            elif "3층" in lowered or "삼층" in lowered:
                mentioned_floor = "3층"
            
            # 전체 층별 안내 요청 또는 특정 층 없을 때 전체 안내
            if show_full_floors or not mentioned_floor:
                if mode == "어린이":
                    return """🏢 어린이과학관 층별 안내예요! 😊
...
#### ⬆️ 2층 — 탐험 출발!
- ✨ **빛놀이터** — 씨앗을 심어 숲을 만드는 신기한 미디어 세상이에요! 에코크리에이터가 되어봐요 🌱
- 🔬 **탐구놀이터** — 에너지·기계·도구를 손으로 직접 만지고 실험할 수 있어요!
- 🦖 **관찰놀이터** — 진짜 공룡 화석이 있어요! 어떤 공룡인지 맞춰봐요!
- 🍱 **휴게실** — 간식 먹고 잠깐 쉬어요. 영유아 전용 놀이터도 있어요 😊
- 🎨 **창작교실** — 토요일에 과학·수학·AI 수업이 열려요

#### ⬇️ 1층 — 과학 체험!
- 🤖 **AI놀이터** — AI 미션을 직접 해결해봐요! 나도 AI 박사가 될 수 있어요!
- 🏃 **행동놀이터** — 점프하고 뛰며 몸으로 과학을 느껴봐요!
- 💡 **생각놀이터** — 내 뇌가 얼마나 똑똑한지 테스트해봐요!
- 🌌 **천체투영관** — 어두운 돔 스크린에서 별자리 여행! *(사전예약 필수)*
- 🎭 **과학극장** — 로봇쇼 또는 사이언스랩 공연 관람! *(무료·선착순)*
- 📚 **꿈트리 동산** — 책 읽고 쉬는 아늑한 공간이에요
- 💊 **의무실·수유실** — 아프거나 아기가 쉬어야 할 때 이용해요

#### 💡 입장 꿀팁!
1. **1층 매표소**에서 표 받기
2. **2층 게이트**로 입장! *(출구는 1층)*
- 다 구경하는 데 약 **2~3시간** 정도 걸려요 ⏰"""
                else:
...
- 과학관 입구(2층)에서 예약한 입장권(QR코드) 확인 후 관람해주시기 바랍니다."""
            ...
            # 특정 층만 짧게 안내
            if mentioned_floor == "1층" and mode == "어린이":
                return """**1층** 안내예요! 😊
...
다른 층 정보도 알려드릴까요?"""
            elif mentioned_floor == "1층" and mode == "성인":
                return """**1층** 안내입니다.
#### 1층
- 매표소·안내데스크
- AI놀이터 / 생각놀이터 / 행동놀이터
...
다른 층 정보도 알려드릴까요?"""
            elif mentioned_floor == "2층" and mode == "어린이":
                return """**2층** 안내예요! 😊
...
다른 층 정보도 알려드릴까요?"""
            elif mentioned_floor == "2층" and mode == "성인":
                return """**2층** 안내입니다.
#### 2층
- 빛놀이터 / 탐구놀이터 / 관찰놀이터
- 창작교실
...
다른 층 정보도 알려드릴까요?"""
            elif mentioned_floor == "3층" and mode == "어린이":
                return """**3층**은 옥상이에요! 😊

🌤️ **하늘마당** — 신선한 공기 마시며 쉬어요

다른 층 정보도 알려드릴까요?"""
            elif mentioned_floor == "3층" and mode == "성인":
                return """**3층**은 옥상입니다.
#### 3층 옥상
- 하늘마당
...
다른 층 정보도 알려드릴까요?"""
        ...
        if category == "facility_amenities":
            ...
            # 전체 편의시설 안내 요청 또는 특정 장소 없을 때 전체 안내
            if show_full_facilities or not mentioned_facility:
                if mode == "어린이":
                    return """🏥 편의시설 안내예요! 😊
#### 1층 로비
- **의무실**: 아플 때 가는 곳이에요
- **수유실·유아휴게실**: 아기들 쉬는 곳이에요
- **물품보관함(락커)**: 물건 보관할 수 있어요
- **유아차(유모차)·휠체어 대여**: 안내데스크에서 빌릴 수 있어요
- **매표소·안내데스크**
- **꿈트리 동산**: 책 읽는 곳이에요
- **자판기**: 음료·간식
...
- **유아차(유모차)·휠체어 대여**: 안내데스크에서 신분증 제시 후 무료 대여
  - 유아차: **유아 36개월 이하까지**
  - **반납 마감: 17:00**
...
- **시설 안전점검**: 법규 기준 이상으로 정기 점검 시행"""
            ...
            # 특정 장소만 짧게 안내
            if mentioned_facility and mode == "어린이":
                return f"""**{mentioned_facility}** 위치를 안내해드릴게요! 😊

📍 **위치**: {facility_locations[mentioned_facility]}

다른 편의시설 정보도 알려드릴까요?"""
            elif mentioned_facility and mode == "성인":
                return f"""**{mentioned_facility}** 위치를 안내해드리겠습니다! 😊
...
다른 편의시설 정보도 알려드릴까요?"""
        ...
        if category == "exhibit_guide":
            ...
            # 특정 놀이터 질문 확인
            exhibit_locations = {
                "ai놀이터": {"floor": "1층", "emoji": "🤖", "desc": "AI 미션을 직접 해결해요! 나도 AI 박사가 될 수 있어요!", "target": "초등학생 추천 / 약 30~40분"},
                "행동놀이터": {"floor": "1층", "emoji": "🏃", "desc": "점프하고 달리며 몸으로 과학을 느껴봐요!", "target": "어린 친구들에게 인기! / 약 20분"},
                "생각놀이터": {"floor": "1층", "emoji": "💡", "desc": "내 뇌가 얼마나 똑똑한지 테스트해봐요!", "target": ""},
                "빛놀이터": {"floor": "2층", "emoji": "✨", "desc": "씨앗을 심어서 디지털 숲을 만드는 신기한 체험이에요!", "target": "누구나 OK! / 약 15분 (시간 맞춰 입장)"},
                "탐구놀이터": {"floor": "2층", "emoji": "🔬", "desc": "에너지랑 기계를 손으로 직접 만져보는 실험이에요!", "target": "초등학생이 특히 좋아해요! / 약 30~40분"},
                "관찰놀이터": {"floor": "2층", "emoji": "🦖", "desc": "진짜 공룡 화석이 있어요! 어떤 공룡인지 맞춰봐요!", "target": "유치원 친구들도 OK! / 약 20분"},
            }
            
            # 질문에 특정 놀이터가 포함되어 있는지 확인
            mentioned_exhibit = None
            for exhibit, info in exhibit_locations.items():
                if exhibit.replace(" ", "") in lowered_no_space:
                    mentioned_exhibit = exhibit
                    break
            
            # 전체 전시관 안내 요청 또는 특정 놀이터 없을 때 전체 안내
            if show_full_exhibits or not mentioned_exhibit:
                if mode == "어린이":
                    return f"""🎨 과학관 놀이터를 소개해요! 😊
...
✨ **빛놀이터** — 씨앗을 심어서 디지털 숲을 만드는 신기한 체험이에요!
> 누구나 OK! / 약 15분 (시간 맞춰 입장)

🔬 **탐구놀이터** — 에너지랑 기계를 손으로 직접 만져보는 실험이에요!
> 초등학생이 특히 좋아해요! / 약 30~40분

---

#### ⬇️ 1층 — 이어서 구경해요!

🤖 **AI놀이터** — AI 미션을 직접 해결해요! 나도 AI 박사가 될 수 있어요!
> 초등학생 추천 / 약 30~40분

🏃 **행동놀이터** — 점프하고 달리며 몸으로 과학을 느껴봐요!
> 어린 친구들에게 인기! / 약 20분

💡 **생각놀이터** — 내 뇌가 얼마나 똑똑한지 테스트해봐요!

🌌 **천체투영관** — 어두운 돔 스크린에서 별자리 여행! ✨
> 4살부터 OK (어른이랑 같이!)  /  꼭 **미리 예약**하세요!

🎭 **과학극장** — 로봇쇼 또는 과학 실험 공연!
> 무료! 선착순이라 일찍 가요 / 약 15분

---

#### 💡 관람 꿀팁
- 천체투영관 시간을 먼저 정하고 그 사이사이에 놀이터 구경하면 딱 좋아요!
- 간식은 2층 **휴게실**에서 먹어요 🍱

💬 특정 놀이터가 더 궁금하면 이름을 말해줘요!"""
                else:
                    return f"""🎨 전시관(놀이터) 소개
...
⚠️ 전시관 구성·운영은 변경될 수 있으니 방문 전 공식 홈페이지에서 확인해 주세요."""
            ...
            # 특정 놀이터만 짧게 안내
            if mentioned_exhibit and mode == "어린이":
                info = exhibit_locations[mentioned_exhibit]
                return f"""**{mentioned_exhibit}** 안내예요! 😊

📍 **위치**: {info['floor']}
{info['emoji']} **{mentioned_exhibit}** — {info['desc']}
> {info['target']}

다른 놀이터 정보도 알려드릴까요?"""
            elif mentioned_exhibit and mode == "성인":
                info = exhibit_locations[mentioned_exhibit]
                return f"""**{mentioned_exhibit}** 안내입니다.
...
다른 놀이터 정보도 알려드릴까요?"""
        ...
        if category == "route_by_age":
            ...
            if _is_weekend:
                if mode == "어린이":
                    _day_notice = f""">**📅 오늘은 {_weekday_kr}요일 (주말)이에요!**
> - 주말에는 사람이 많아서 **일찍 도착하는 게 좋아요!** (문 열기 10분 전쯤 와요 😊)
> - **천체투영관·빛놀이터는 꼭 미리 예약해야 해요!** — 당일 남은 자리가 거의 없어요
> - **과학쇼·전시해설**도 자리가 빨리 없어지니 일찍 가요! (공연 시작 25분 전부터 입장)
> - 로봇이 돌아다니는 로봇순회는 주말에 없어요"""
                else:
                    _day_notice = f""">**📅 오늘은 {_weekday_kr}요일 (주말)입니다.**
> - 주말은 방문객이 많아 **일찍 도착**을 강력 권장합니다 (개관 10분 전 대기 추천!)
> - **천체투영관·빛놀이터는 사전예약 거의 필수** — 현장 잔여석이 매우 적습니다
> - **과학쇼·전시해설** 선착순 자리도 일찍 잡으십시오 (25분부터 입장)
> - 로봇순회는 주말에 운영하지 않습니다"""
            elif _is_weekday:
                if mode == "어린이":
                    _robot_tour_note = "**로봇순회(꿈트리동산, 14:30)**도 있어요 — 로봇이 직접 돌아다니는 걸 볼 수 있어요!" if not _is_vacation else "방학이라 평일도 사람이 많을 수 있어요 — 일찍 도착하는 게 좋아요!"
                    _day_notice = f""">**📅 오늘은 {_weekday_kr}요일 (평일)이에요!**
> - 주말보다 여유로워요! {_robot_tour_note}
> - 천체투영관·빛놀이터 자리가 평일엔 여유 있어요 (그래도 미리 예약하면 더 좋아요!)
> - **과학쇼·전시해설** 자리 잡기가 주말보다 쉬워요 😊"""
                else:
                    _robot_tour_note = "**로봇순회(꿈트리동산, 14:30)**가 운영됩니다 — 줄 없이 여유롭게 관람 가능!" if not _is_vacation else "방학기간이라 평일도 붐빌 수 있으니 — 일찍 도착을 권장합니다!"
                    _day_notice = f""">**📅 오늘은 {_weekday_kr}요일 (평일)입니다.**
> - 주말보다 여유롭고 {_robot_tour_note}
> - 천체투영관·빛놀이터 현장 잔여석 가능성이 높습니다 (사전예약 우선)
> - **과학쇼·전시해설** 선착순이지만 주말보다 경쟁이 덜합니다"""
            else:
                if mode == "어린이":
                    _day_notice = "> **📅 오늘은 월요일(휴관일)이에요.** 내일(화요일)부터 이 동선을 활용해 보세요!"
                else:
                    _day_notice = "> **📅 오늘은 월요일(휴관일)입니다.** 내일(화요일)부터 이 동선을 활용해 보십시오!"

            if mode == "어린이":
                return f"""🗺️ 나이에 맞는 관람 코스예요! 😊

{_day_notice}

---

#### 👶 4~7살 (유치원 친구들) — 약 2시간~2시간 30분

1. 🦖 **관찰놀이터** (2층) — 공룡 화석 탐험! (약 25분)
2. ✨ **빛놀이터** (2층) — 씨앗으로 숲 만들기! (약 30분)
3. 🍪 **2층 휴게실** — 간식 먹고 쉬어요 (약 20분)
4. 🏃 **행동놀이터** (1층) — 점프하고 달려봐요! (약 25분)
5. 🎭 **과학극장** (1층) — 로봇쇼 또는 과학 실험 관람! (15분)

> 💡 천체투영관은 4살부터 OK! 어른과 꼭 같이 가요.

---

#### 🎒 초등 저학년 (8~10살) — 약 2시간 30분~3시간

1. 🔬 **탐구놀이터** (2층) — 에너지·기계 직접 실험! (약 40분)
2. 🦖 **관찰놀이터** (2층) — 공룡 화석 구경 (약 20분)
3. 🎭 **과학극장** (1층) — 로봇쇼 또는 사이언스랩 (15분)
4. 🏃 **행동놀이터** (1층) — 몸으로 과학 체험 (약 20분)
5. 🌌 **천체투영관** (1층) — 별자리 여행! (**꼭 미리 예약!**)

> 💡 {'주말이라 과학쇼(11:30) 자리를 먼저 잡고 전시관 구경하세요!' if _is_weekend else '평일이라 여유로워요! 로봇순회(14:30, 꿈트리동산)도 챙겨보세요!'}

---

#### 🧑‍🎓 초등 고학년 (11~13살) — 약 2시간 30분~3시간

1. 🔬 **탐구놀이터** (2층) — 원리 탐구 집중! (약 40분)
2. 🤖 **AI놀이터** (1층) — AI 미션 해결! (약 40분)
3. 🎙️ **전시해설** (15분) — 선생님이 공룡이나 새에 대해 재미있게 설명해줘요!
4. 🌌 **천체투영관** (1층) — 별자리 여행! (**꼭 미리 예약!**)

> 💡 {'주말이라 천체투영관 자리가 빨리 없어져요. 들어오자마자 예약 먼저!' if _is_weekend else '평일이라 여유로워요! AI놀이터 미션을 천천히 즐길 수 있어요.'}

---

#### 👨‍👩‍👧 나이 차이 나는 형제자매가 함께 왔다면?
**2층 빛놀이터·관찰놀이터**에서 시작하면 누구나 함께 즐길 수 있어요!
쉬는 시간은 2층 **휴게실**에서 🍱

#### 🎟️ 꼭 기억해요!
- **천체투영관은 꼭 미리 예약!**
- {'주말이라 예약은 미리 해오는 게 좋아요!' if _is_weekend else '평일은 당일 표 사기도 여유로워요 (그래도 예약이 더 좋아요)'}

💬 "6살인데 어디부터 볼까요?" 처럼 **나이**를 말해주면 딱 맞는 코스를 알려드릴게요!"""
            else:
...
자녀 연령에 맞춰 과학관을 효율적으로 돌아볼 수 있는 동선을 추천해드립니다. **전체 관람 시간은 2~3시간** 정도가 적당합니다.
...
{'- 🌟 **오늘(주말) 꿀팁**: 빛놀이터·천체투영관은 개관 직후 예약 마감될 수 있습니다. 입장 즉시 예약 부스로!' if _is_weekend else '- 🌟 **오늘(평일) 꿀팁**: 로봇순회(14:30)가 운영되니 꿈트리동산을 들러보십시오!'}
...
- '전시해설(「헬로 다이노!」/「짹짹 새 탐험대」)' 프로그램도 15분 정도로 좋습니다.
{'- 🌟 **오늘(주말) 꿀팁**: 과학쇼(11:30)를 먼저 잡고 전시 관람하세요. 대기열이 빨리 찹니다!' if _is_weekend else '- 🌟 **오늘(평일) 꿀팁**: 로봇순회(14:30, 꿈트리동산)와 과학쇼(11:30/13:30) 모두 챙길 수 있습니다!'}
...
- 남는 시간이 있다면 1층 **행동놀이터**로 에너지를 발산해도 좋습니다.
{'- 🌟 **오늘(주말) 꿀팁**: 천체투영관 14:00 회차는 인기가 많습니다. 당일 현장 잔여석을 빠르게 확인하세요!' if _is_weekend else '- 🌟 **오늘(평일) 꿀팁**: 관람객이 적어 AI놀이터 미션을 여유롭게 즐길 수 있습니다. 로봇순회(14:30)도 추가하세요!'}
...
💬 "6살인데 어디부터 보면 좋을까?" 처럼 **구체적 나이와 현재 위치(1층/2층)**를 말해주시면 더 정확한 동선을 짜드리겠습니다!"""
        ...
        if category == "today_programs":
            ...
            if mentioned_program:
                ...
                if mentioned_program == "로봇쇼":
                    if mode == "어린이":
                        return f"""**로봇쇼**는 **1층 과학극장**에서 해요! 🎉
...
다른 오늘의 프로그램 정보도 알려드릴까요?"""
                    else:
                        return f"""**로봇쇼**는 **1층 과학극장**에서 진행됩니다! 🎉
...
다른 오늘의 프로그램 정보도 알려드릴까요?"""
                elif mentioned_program == "사이언스랩":
                    science_lab_theme = {
                        1: "비눗방울", 3: "비눗방울", 5: "비눗방울",
                        7: "공기", 9: "힘", 11: "공기"
                    }.get(month, "")
                    theme_text = f" (테마: {science_lab_theme})" if science_lab_theme else ""
                    if mode == "어린이":
                        return f"""**사이언스랩**은 **1층 과학극장**에서 해요! 🎉{theme_text}
...
다른 오늘의 프로그램 정보도 알려드릴까요?"""
                        ...
                        return f"""**사이언스랩**은 **1층 과학극장**에서 진행됩니다! 🎉{theme_text}
...
다른 오늘의 프로그램 정보도 알려드릴까요?"""
                elif mentioned_program == "전시해설":
                    if month in [1, 3, 5, 7, 9, 11]:
                        # 홀수월: 헬로 다이노
                        if mode == "어린이":
                            return f"""**전시해설**은 {date_label} 진행돼요! 😊
...
다른 오늘의 프로그램 정보도 알려드릴까요?"""
                            ...
                            return f"""**전시해설**은 {date_label} 진행됩니다! 😊
...
다른 오늘의 프로그램 정보도 알려드릴까요?"""
            ...
            if _time_match:
                ...
                if _available:
                    ...
                    if mode == "어린이":
                        ...
                        return (
                            f"**{_time_str}에 입장하시면 참여 가능한 프로그램입니다!** 🎉\n\n"
                            f"{_lines}\n\n"
                            f"> 💡 무료 프로그램(과학쇼·전시해설)은 선착순이라 일찍 자리를 잡으세요!\n"
                            f"> 천체투영관·빛놀이터는 [신청콕](https://www.sciencecenter.go.kr/csc/reserve/individual) 사전예약을 권장합니다."
                        )
                else:
                    if mode == "어린이":
                        return f"**{_h}시 이후에는 참여 가능한 정기 프로그램이 마감됐어요.** 😢\n\n과학관 운영시간은 09:30~17:30(입장 마감 16:30)이에요. 전시 관람은 여전히 가능해요!"
                    else:
                        return f"**{_h}시 이후에는 참여 가능한 정기 프로그램이 마감되었습니다.** 😢\n\n과학관 운영시간은 09:30~17:30(입장 마감 16:30)입니다. 전시 관람은 여전히 가능합니다."
            ...
            if is_closed:
                ...
                if mode == "어린이":
                    ...
                    return (
                        f"**{target_date.strftime('%Y년 %m월 %d일')} {weekday_kr}요일입니다.**\n\n"
                        f"{date_label}은 휴관일이라 프로그램이 운영되지 않습니다. 😢\n\n"
                        f"#### 국립어린이과학관 운영 안내\n"
                        f"- **운영일**: 화요일 - 일요일\n"
                        f"- **휴관일**\n"
                        f"  - 매주 월요일\n"
                        f"  - 1월 1일, 설날 당일, 추석 당일\n"
                        f"  - 월요일이 공휴일인 경우 개관하며, 화요일에 대체 휴관\n\n"
                        f"**다음 개관일은 {next_open.strftime('%m월 %d일')}({next_open_wd}요일)입니다! 🎉**\n"
                        f"운영일에 진행되는 프로그램(과학쇼, 전시해설, 천체투영관 등)이 궁금하시면 말씀해 주세요!"
                    )
...
* 프로그램 내용 및 시간표는 운영 상황에 따라 변동될 수 있어요. 공휴일은 주말 일정과 동일하게 운영해요."""
                else:
                    return """전시해설 안내입니다. 😊
...
#### 프로그램(월별)
- **스펫해설 「헬로 다이노!」** (1, 3, 5, 7, 9, 11월)
  - 백악기 시대 공룡 이야기로 과학을 쉽고 재미있게 알아봅니다.
  - 장소: 2층 공룡 전시물 앞
  - 참여방법: 자유 관람
- **전시톡톡해설 「짹짹 새 탐험대」** (2, 4, 6, 8, 10, 12월)
  - 다양한 새들의 특징을 퀴즈로 알아봅니다.
  - 장소: 1층 과학극장
  - 참여방법: 선착순 입장 및 관람
  - 입장 안내: 25분부터 입장 가능 / 29분 59초에 입장 마감
  - 진행 중에는 입장/퇴장 불가
...
* 프로그램 내용 및 시간표는 운영 상황에 따라 변동될 수 있습니다. 공휴일은 주말 일정과 동일하게 운영합니다."""
...
- 어떤 곳인가요?
  - 돔 스크린으로 별자리/우주 영상을 관람하고 해설을 듣는 프로그램입니다.

- 예약/입장
  - 예약이 필요한 경우가 많고, 회차별 정원이 있습니다.

원하시면 '오늘 천체투영관 시간' 또는 '천체투영관 예약 방법'이라고 물어보면 더 자세히 안내해드리겠습니다."""
...
> 💡 **과학교실·수학교실 등 교육 수업**이 궁금하시면 "오늘 교육프로그램 알려줘"라고 물어봐주세요!"""
                ...
                return f"""**{target_date.strftime('%Y년 %m월 %d일')} {weekday_kr}요일입니다!**

{date_label}(요약) 프로그램 시간표를 안내해드리겠습니다. *(운영 상황에 따라 변동될 수 있습니다.)*
...
원하시면 "과학쇼 자세히", "전시해설 자세히", "천체투영관 시간표", "빛놀이터 시간"처럼 말해주면 안내 규정/입장방법까지 더 자세히 설명해드리겠습니다.

> 💡 **과학교실·수학교실 등 교육 수업**이 궁금하시면 "오늘 교육프로그램 알려줘"라고 물어봐주세요!"""

        if category == "night_observation":
            if mode == "어린이":
                return """🌙 「서울에서 별 본다」 천체관측 프로그램 안내예요!

> ⚠️ **현재 공사로 인해 프로그램이 중단 중이에요.**
...
"""
            ...
            _round_map = [
                ...
                (["길냥이"], "길냥이 키츠 시리즈", "2·5회차(11:00, 15:00)"),
            ]
            ...
            # 전체 시간표 요청이고 특정 프로그램이 없을 때만 전체 안내
            # 특정 프로그램이 언급되면 전체 시간표 요청 키워드가 있어도 특정 프로그램만 안내
            if show_full_timetable and not mentioned_program:
                _prog_prefix = ""
                if mentioned_program:
                    if mode == "어린이":
...
#### 🚶‍♂️ 이동 안내
- 바닥에 있는 **주황색 선**을 따라가시면 천체투영관으로 갈 수 있어요!
...
#### 🚶‍♂️ 이동 안내
- 바닥에 있는 **주황색 선**을 따라가시면 천체투영관으로 갈 수 있습니다.
...
def answer_rule_based_localized(intent: str, message: str, mode: str, language: str) -> tuple[str, str]:
    ...
    # 모드별 말투 조정 (한국어 원문에 적용)
    ko_answer = adjust_tone_for_mode(ko_answer, mode, "한국어")
    ...
    # 폴백: LLM 번역 (mode 전달 → 어린이 모드일 때 child-friendly 번역 힌트)
    translated = translate_answer_cached(ko_answer, language, mode)
    return translated, ko_answer
...
⚠️ 국립어린이과학관에는 전용 주차장이 없습니다. 반드시 대중교통을 이용해 주세요.

[목적지 정보]
- 주소: 서울특별시 종로구 창경궁로 215
- 가장 가까운 지하철역: 혜화역 (4호선, 도보 약 7분)

[경로 안내 — 아래 정확한 환승역 정보를 반드시 사용할 것]
목적지는 4호선 혜화역입니다.
4호선으로 갈아탈 수 있는 환승역(정확한 정보):
- 동대문역사문화공원역: 2호선 ↔ 4호선 ↔ 5호선 환승 가능
- 충무로역: 3호선 ↔ 4호선 환승 가능
- 동대문역: 1호선 ↔ 4호선 환승 가능
- 서울역: 1호선 ↔ 4호선 환승 가능
- 사당역: 2호선 ↔ 4호선 환승 가능

{origin}에서 위 환승역 중 하나를 경유하는 개략적 경로를 안내하세요.
위 환승역 목록에 없는 역에서 4호선으로 환승할 수 있다고 절대 안내하지 마세요.
"참고용 경로이며 정확한 소요시간은 지도 앱에서 확인 바랍니다"라는 안내를 포함하세요.
...
def get_latest_notices_text(limit: int = 5) -> str:
    ...
    # 캐시 파일에서 공지사항 읽기
    cache_path = Path(__file__).parent / "data" / "notices_cache.json"
    try:
...
=== 대화 기록 참조 지시 ===
- 사용자의 이전 질문과 시스템의 이전 답변을 반드시 확인하세요.
- "몇 시?", "언제?", "어디?", "어디야?" 같은 질문은 직전 대화 맥락을 참조하여 답변하세요.
- 예: "프로그램 물어봤어" → "오늘의 프로그램 안내" 후 "몇 시야?" → 해당 프로그램의 시간을 답변
- 대화 기록에 충분한 정보가 없으면 정중하게 "어떤 프로그램/시설에 대해 말씀하시는 건가요?"라고 물어보세요.
"""
...
- 전시해설 ({_docent_name}) / {_docent_place}: {_docent_today} / 무료 선착순"""
        ...
        if _weekday_idx == 0:  # 월 (휴관)
            ...
            _show_today = "(월요일 휴관)"
        elif _weekday_idx in (5, 6):  # 토~일: 전시해설 운영
            _docent_today = "10:30, 14:30, 15:30"
            _show_today = "11:30, 13:30"
        else:  # 화~금: 전시해설 미운영 (단체해설 10:40 화~금 예약제)
            _docent_today = "오늘(평일)은 전시해설 미운영 — 주말(토/일)에만 10:30, 14:30, 15:30 운영 / 단체해설(10:40)은 화~금 예약제로 별도 진행"
            _show_today = "11:30, 13:30"
        _today_schedule = f"""[{_month}월 오늘({weekday_kr}요일)의 프로그램 시간표]
...
사용자가 "다이노"라고만 질문하면 다음과 같이 명확히 구분해서 물어보세요:
"다이노에 대해 물어보셨네요! 정확히 안내드리기 위해 한 가지만 더 물어볼게요. 😊

어떤 것을 찾으시나요?

- 다이노 터널(전시물) — 2층 관찰놀이터에 있는 공룡 터널 전시물
- 다이노소어 프로그램 — 천체투영관에서 상영하는 공룡 체험 프로그램"
...
=== ⚠️ 방문 활동 추천 지침 (매우 중요) ===
사용자가 "뭐 하면 돼?", "몇 살 아이랑 왔는데 뭐가 좋아?", "추천해줘" 등 현장 활동을 물어볼 때:

**절대 추천 금지 — 교육프로그램 (과학교육, 창작교실, 교육신청 등)**
- 이유: 교육프로그램은 사전 온라인 예약제이며, 예약이 열리자마자 마감됩니다. 당일 현장에서는 참여가 거의 불가능합니다.
- 교육프로그램은 사용자가 **직접 교육 관련 질문**을 할 때만 안내하세요.

**⛔ 아래 4가지는 하나라도 빠뜨리면 안 됩니다. 반드시 모두 포함하세요. (search_exhibit_info 도구 호출 금지)**

1. **상설전시관 관람** ← 반드시 포함
   - 1층: 행동놀이터, 생각놀이터, AI놀이터
   - 2층: 탐구놀이터, 관찰놀이터
...
@st.cache_data(show_spinner=False, ttl=60 * 60 * 24)
def translate_answer_cached(text: str, target_language: str, mode: str = "성인") -> str:
    ...
    tone_hint = (
        " Use simple, friendly, child-appropriate language with a warm and encouraging tone."
        if mode == "어린이" else ""
    )
    try:
        ...
        prompt = (
            f"Translate the following Korean text into {lang_label}.{tone_hint} "
